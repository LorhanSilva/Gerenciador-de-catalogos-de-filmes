...
TFM *cria_filme(char* nome, char* diretor, char* genero, int lancamento, int duracao){
    TFM *f = (TFM*)malloc(sizeof(TFM));
    strcpy(f->titulo, nome);
...
int hash_filme(char* titulo, int data){
    int h=0, i, len = strlen(titulo);
    for(i=0;i<len;i++){
        h += ((int)titulo[i]) + i+1;
    }
    h = h *10000 + (data + len) ;
...
TARVB *TARVB_Cria(int t){
  ...
  novo->filmes = (TFM**)malloc(sizeof(TFM*)*((t*2)-1));
  novo->folha=1;
...
TARVB *TARVB_Libera(TARVB *a){
  if(a){
    ...
    for(int i=0;i<a->nchaves;i++)free(a->filmes[i]);
    free(a->filmes);
    free(a->chave);
    free(a->filho);
    free(a);
...
TARVB *Busca_filmes(TARVB* x, int ch, int *pos){ //Busca para filmes: pos = ponteiro que vai ser a posição do filme no vetor.
  if(!x) return NULL;
  int i = 0;
  ...
  return Busca_filmes(x->filho[i], ch, pos);
}
...
TARVB *Divisao(TARVB *x, int i, TARVB* y, int t){
  ...
  for(j=x->nchaves; j>=i; j--){
    ...
    x->filmes[j] = x->filmes[j-1];
  }
  x->chave[i-1] = y->chave[t-1];
  x->filmes[i-1] = y->filmes[t-1];
  x->nchaves++;
  return x;
}


TARVB *Insere_Nao_Completo(TARVB *x, int k, TFM* f, int t){
  int i = x->nchaves-1;
  if(x->folha){
    while((i>=0) && (k<x->chave[i])){
      x->chave[i+1] = x->chave[i];
      x->filmes[i+1] = x->filmes[i];
      i--;
    }
    x->chave[i+1] = k;
    x->filmes[i+1] = f;
  ...
  x->filho[i] = Insere_Nao_Completo(x->filho[i], k, f, t);
  return x;
}


TARVB *TARVB_Insere(TARVB *T, int k, TFM* f, int t){
  ...
  if(T->nchaves == (2*t)-1){
    ...
    S->filho[0] = T;
    S = Divisao(S,1,T,t);
    S = Insere_Nao_Completo(S,k,f,t);
    return S;
  }
  T = Insere_Nao_Completo(T,k,f,t);
  return T;
}
...
TARVB* remover(TARVB* arv, int ch, int t){
  ...
  if(i < arv->nchaves && ch == arv->chave[i]){ //CASOS 1, 2A, 2B e 2C
    ...
    if(!arv->folha && arv->filho[i]->nchaves >= t){ //CASO 2A
      ...
      TFM *ftemp = y->filmes[y->nchaves-1];
  ...
  if (y->nchaves == t-1){ //CASOS 3A e 3B
    if((i < arv->nchaves) && (arv->filho[i+1]->nchaves >=t)){ //CASO 3A
      ...
      y->filmes[t-1] = arv->filmes[i];
      y->nchaves++;
      arv->chave[i] = z->chave[0];     //dar a arv uma chave de z
      arv->filmes[i] = z->filmes[0];
      int j;
      for(j=0; j < z->nchaves-1; j++){  //ajustar chaves de z
        z->chave[j] = z->chave[j+1];
        z->filmes[j] = z->filmes[j+1];
      }
